...
- Tiny userland build pipeline with syscall headers, startup assembly, a user linker script, pattern-rule-driven compiled `demo` and `counter` ELFs, and `objcopy` embedding into the kernel
...
- First VFS-style file object layer with open, read, seek, close, and per-process file descriptor ownership
- User-facing file syscalls with `SYS_OPEN`, file-backed `SYS_READ`, `SYS_SEEK`, and `SYS_CLOSE`
- Shell `cat PATH` and `seektest PATH` support for reading and repositioning named RAMFS files from user mode
