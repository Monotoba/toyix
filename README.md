...
- First VFS-style file object layer with open, read, close, and per-process file descriptor ownership
- User-facing file syscalls with `SYS_OPEN`, file-backed `SYS_READ`, and `SYS_CLOSE`
- Shell `cat PATH` support for reading named RAMFS files from user mode
